...
def generate_coldstart(
    ...
    tfidf_matrix = (
        ...
        if fit_new
        else tfidf_vectorizer.transform(text_data)
    )

    profile_keywords = user_prefs.get("diet", []) + user_prefs.get("region_pref", [])
    ...
    if avoid_filter:
        ranked_df = recipe_avoid_filter(ranked_df, user_prefs)
